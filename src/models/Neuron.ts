...
export class Neuron {
  static startsAtSensoryNeuron = ({
    ...
    return (
      ...
      || !!inputConnections?.find((c) => {
        return Neuron.startsAtSensoryNeuron(c.from);
      })
    );
  };
  ...
  static endsAtMotorNeuron = ({ type, outputConnections }: Neuron): boolean => {
    return (
      ...
      || !!outputConnections?.find((c) => {
        return Neuron.endsAtMotorNeuron(c.to);
      })
    );
  };
  ...
  constructor(neuron: SensoryNeuron | MotorNeuron | InnerNeuron) {
    ...
    this._type = (_inputFunction) ? NeuronType.SENSORY : (
      (_outputFunction) ? NeuronType.MOTOR : NeuronType.INNER
    );
  ...
  getOutputValue(): number {
    /**
     * think about buffering value in prop and transform this into a real getter
    ...
    if (this._inputFunction) {
      ...
      return tanh(
        this._inputConnections
          .map((c) => {
            return c.from.getOutputValue() * c.weight;
          })
          .reduce((a, b) => {
            return a + b;
          }, 0),
  ...
  isDependingOn(neuron: Neuron): boolean {
    return (
      this === neuron
      || !!(this.inputConnections || []).find((c) => {
        return c.from.isDependingOn(neuron);
      })
    );
  }
  ...
  toJSON() {
    return {
      id: this._id,
      type: this._type,
      inputConnections: this._inputConnections?.map((c) => {
        return c.toJSON();
      }),
      outputConnections: this._outputConnections?.map((c) => {
        return c.toJSON();
      }),
    };
  }
